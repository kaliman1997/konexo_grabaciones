socket.on('chat_historico_response', function (msg) {

	$('#chat_historico_list').DataTable().ajax.reload();
	setTimeout(function(){
...
$(document).ready(function ($) {
	...
	var chat_historico_list = $("#chat_historico_list").on('error.dt', function (e, settings, techNote, message) {
		...
		ajax           : {
			...
			type   : "POST",
			data   : {
				reportFilter: 'chatColasConUmbralHistorico'
			},
			dataSrc: ""
		},
		columns        : [
		...
		var data = chat_historico_list.row($(this)).data();
	...
	$('#chat_historico_submit').click(function () {

		$('#overlay_page').css("display", 'block');

		var startDate = $('#start_date_chat_historico').val();
		var finalDate = $('#final_date_chat_historico').val();

		$.post("/ChatColasConUmbralHistorico", {
	...
	$('#start_date_chat_historico, #final_date_chat_historico').val(today.format('YYYY-MM-DD, HH:mm'));

	$('#start_range_label_chat_historico').attr("placeholder",
		todayOneWeekAgo.format("MMM DD, YYYY, HH:mm") + " --- " + today.format("MMM DD, YYYY, HH:mm")
	);

	$('#start_date_chat_historico').val(todayOneWeekAgo.format('YYYY-MM-DD HH:mm'));
	$('#final_date_chat_historico').val(today.format('YYYY-MM-DD HH:mm'));

	$('#start_range_label_chat_historico').daterangepicker({
		parentEl           : "title_div",
		showDropdowns      : true,
		minYear            : "2021",
		minDate            : "2021-01-29",
		...
		$('#start_date_chat_historico').val(start.format('YYYY-MM-DD HH:mm'));
		$('#final_date_chat_historico').val(end.format('YYYY-MM-DD HH:mm'));
	});
});
