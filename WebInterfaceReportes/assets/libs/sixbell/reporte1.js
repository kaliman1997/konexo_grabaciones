var filterTypeGlobal    = "byAgent";
var filterTypeGlobalURL = "/AgentList";
var agentUniverse       = [];
var groupUniverse       = [];
	...
	columns        : [
		{
			...
			render       : function (data, type, row) {
				return "&nbsp;&nbsp;&nbsp;&nbsp;<input id='" + data + "' name='agent' onChange='addItem()' type='radio' value='" + data + "' style='margin-left: 15px !important;'>";
			},
			sTitle       : "",
			sWidth       : "20px",
			bSortable    : true,
...
socket.on('reporte_1', function (msg) {
...
function addItem() {
	...
	source_list.rows().every(function (rowIdx, tableLoop, rowLoop) {
		...
		if ($('#' + row_data.id).is(':checked')) {
			...
			if (filterTypeGlobal === 'byAgent') {

				let userObject = {
					userId: $('#' + row_data.id).val()
				};
				agentUniverse.push(userObject);
			}
			else {

				let groupObject = {
					groupId: $('#' + row_data.id).val()
				};
				groupUniverse.push(groupObject);
			}
		}
	});

	console.log("Agentes: " + JSON.stringify(((filterTypeGlobal === 'byAgent') ? agentUniverse : groupUniverse), null, 4));
}
...
$(document).ready(function ($) {
			...
			columns        : [
				...
				},
				{
					...
					render        : function (data, type, row) {
						...
						return "<button type='button' class='btn btn-red btn-icon btn-icon-standalone btn-xs' onClick='window.location.href = \"/ReportStorage/"+$('#userId').val()+"/Reporte1/" + data + "\"'>" +
	...
	$('#filter_by_user').click(function () {
		...
		$('#filter_by_group_label').removeClass('text-danger');
		$('#filter_by_user_label').removeClass('text-danger').addClass('text-danger');
		dataTableOptions.ajax.url = "/AgentList";

		filterTypeGlobal = "byAgent";
	...
	$('#filter_by_group').click(function () {

		var agentUniverse = [];
		var groupUniverse = [];

		$('#filter_by_user_label').removeClass('text-danger');
		$('#filter_by_group_label').removeClass('text-danger').addClass('text-danger');
		dataTableOptions.ajax.url = "/GroupList";

		filterTypeGlobal = "byGroup";
		var source_list  = $("#source_list").on('init.dt', function () {
	...
	$('#report_submit').click(function () {
		...
		var searchType = (filterTypeGlobal === 'byAgent') ? 'user' : 'group';
		var searchData = (filterTypeGlobal === 'byAgent') ? agentUniverse : groupUniverse;
	...
	$('#start_range_label').datepicker({
		...
		$('#start_date').val(moment(ev.date).format('YYYY-MM-DD') + " 00:00");
		$('#final_date').val(moment(ev.date).format('YYYY-MM-DD') + " 23:59");
	});/*
