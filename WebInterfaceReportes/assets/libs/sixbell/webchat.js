socket.on('webchat_response', function (msg) {

	$('#webchat_list').DataTable().ajax.reload();
	setTimeout(function () {
...
$(document).ready(function ($) {
	var consolidado_list = $("#webchat_list").on('error.dt', function (e, settings, techNote, message) {
	...
	$('#webchat_submit').click(function () {

		$('#overlay_page').css("display", 'block');

		var startDate = $('#start_date_webchat').val();
		var finalDate = $('#final_date_webchat').val();
		var data      = {
	...
	$('#start_date_webchat, #final_date_webchat').val(today.format('YYYY-MM-DD, HH:mm'));

	$('#start_range_label_webchat').attr("placeholder",
		todayOneWeekAgo.format("MMM DD, YYYY, HH:mm") + " --- " + today.format("MMM DD, YYYY, HH:mm")
	);

	$('#start_date_webchat').val(todayOneWeekAgo.format('YYYY-MM-DD HH:mm'));
	$('#final_date_webchat').val(today.format('YYYY-MM-DD HH:mm'));

	$('#start_range_trigger_webchat').daterangepicker({
		parentEl           : "title_div",
		showDropdowns      : true,
		minYear            : "2021",
		minDate            : "2021-01-01",
		...
		$('#start_date_webchat').val(start.format('YYYY-MM-DD HH:mm'));
		$('#final_date_webchat').val(end.format('YYYY-MM-DD HH:mm'));
	});
});
