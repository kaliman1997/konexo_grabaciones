...
socket.on('voz_tiempo_real_response', function (msg) {

	$('#voz_tiempo_real_list').DataTable().ajax.reload();
	setTimeout(function(){
		...
		$.ajax({
			...
			error: function()
			{
				$('#download_error_modal').modal('show', modal_opts).detach().appendTo("body");
			},
			success: function()
			{
...
$(document).ready(function ($) {
	var voz_tiempo_real_list = $("#voz_tiempo_real_list").on('error.dt', function (e, settings, techNote, message) {
		...
		ajax           : {
			...
			type   : "POST",
			data   : {
				reportFilter: 'vozColasConUmbralTiempoReal'
			},
			dataSrc: ""
		},
		columns        : [
		...
		var data = voz_tiempo_real_list.row($(this)).data();
	...
	$('#voz_tiempo_real_submit').click(function () {

		$('#overlay_page').css("display", 'block');

		var startDate = $('#start_date_voz_tiempo_real').val();
		var finalDate = $('#final_date_voz_tiempo_real').val();

		$.post("/VozColasConUmbralTiempoReal", {
	...
	$('#start_date_voz_tiempo_real').val(todayOneHourAgo.format('YYYY-MM-DD HH:mm'));
	$('#final_date_voz_tiempo_real').val(today.format('YYYY-MM-DD HH:mm'));

	$('#start_range_label_voz_tiempo_real').daterangepicker({
		parentEl           : "title_div",
		showDropdowns      : true,
		minYear            : "2021",
		minDate            : "2021-01-29",
		...
		$('#start_date_voz_tiempo_real').val(start.format('YYYY-MM-DD HH:mm'));
		$('#final_date_voz_tiempo_real').val(end.format('YYYY-MM-DD HH:mm'));
	});
});
