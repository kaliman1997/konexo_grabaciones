socket.on('voz_historico_response', function (msg) {
	...
	$('#voz_historico_list').DataTable().ajax.reload();
	setTimeout(function(){
...
$(document).ready(function ($) {
	var voz_historico_list = $("#voz_historico_list").on('error.dt', function (e, settings, techNote, message) {
		...
		ajax           : {
			...
			type   : "POST",
			data   : {
				reportFilter: 'vozColasConUmbralHistorico'
			},
			dataSrc: ""
		},
		columns        : [
		...
		var data = voz_historico_list.row($(this)).data();
	...
	$('#voz_historico_submit').click(function () {
		...
		$.post("/VozColasConUmbralHistorico", {
