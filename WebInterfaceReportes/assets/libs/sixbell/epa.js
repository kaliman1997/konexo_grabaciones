socket.on('epa_response', function (msg) {

	console.log(msg);

	$('#epa_list').DataTable().ajax.reload();
	setTimeout(function () {
...
$(document).ready(function ($) {
	var consolidado_list = $("#epa_list").on('error.dt', function (e, settings, techNote, message) {
	...
	$('#epa_submit').click(function () {

		$('#overlay_page').css("display", 'block');

		var startDate = $('#start_date_epa').val();
		var finalDate = $('#final_date_epa').val();
		var data      = {
	...
	$('#start_date_epa, #final_date_epa').val(today.format('YYYY-MM-DD, HH:mm'));

	$('#start_range_label_epa').attr("placeholder",
		todayOneWeekAgo.format("MMM DD, YYYY, HH:mm") + " --- " + today.format("MMM DD, YYYY, HH:mm")
	);

	$('#start_date_epa').val(todayOneWeekAgo.format('YYYY-MM-DD HH:mm'));
	$('#final_date_epa').val(today.format('YYYY-MM-DD HH:mm'));

	$('#start_range_trigger_epa').daterangepicker({
		parentEl           : "title_div",
		showDropdowns      : true,
		minYear            : "2021",
		minDate            : "2021-01-01",
		...
		$('#start_date_epa').val(start.format('YYYY-MM-DD HH:mm'));
		$('#final_date_epa').val(end.format('YYYY-MM-DD HH:mm'));
	});
});
